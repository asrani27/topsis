...
<div class="max-w-2xl mx-auto">
    <!-- Page Header -->
    <div class="mb-6">
        <h1 class="text-2xl sm:text-3xl font-bold text-gray-800">Edit Kriteria</h1>
        <p class="text-gray-600 mt-1">Perbarui data kriteria</p>
    </div>
        ...
        <form action="{{ route('admin.kriteria.update', $kriteria) }}" method="POST">
            ...
            <div class="mb-6">
                ...
                >
                    ...
                    <option value="benefit" {{ old('tipe', $kriteria->tipe) == 'benefit' ? 'selected' : '' }}>Benefit (Makin tinggi makin baik)</option>
                    <option value="cost" {{ old('tipe', $kriteria->tipe) == 'cost' ? 'selected' : '' }}>Cost (Makin rendah makin baik)</option>
                </select>
                @error('tipe')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
